...
- 最终启动编排包含：最早期 WMI 运行钩子、隔离宿主原生依赖、WebEngine 子进程输出临时文件、父进程延后 WebEngine 导入、IPC 拒绝访问自动单进程兼容、仅创建“明渠设计”首屏、主窗口显式退出控制。相关定向回归为 `18 passed`。
- 已排除同期未提交的有压管道改动后重建最终急用包：`output/CanalHydraulicCalc-V1.3.11-急用.zip` 为 378,716,549 字节，SHA-256 `9279EC34CA78492EA674E0E08FCA3CE69A0EAD4390589DEB9D1E88A3EC8ED99D`；ZIP 共 5256 项、CRC 正常且无 Codex/Poppler/UCRT 污染。全新解压后在普通 Windows 桌面连续两次确认标题 `渠系建筑物水力计算系统 V1.3.11`、窗口 1415×937、进程响应正常。GitHub Release 与 Gist 待替换为该最终资产。
...
- GitHub V1.3.11 曾替换过一轮中间修正版，但该远端资产已被后续“主窗口刚显示就消失”复现判定为过期；必须以本轮最终哈希回读结果为准。
