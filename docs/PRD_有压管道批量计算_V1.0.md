...
水锤专用窗口内的“验算/刷新”采用橙色强调按钮，便于用户识别需要主动执行的校核动作；验算结果区按“数值紧凑网格 + 长结论整行换行”展示，`控制来源 / 控制类型 / 图1-3-3对照` 等长文本不得撑宽窗口，也不依赖横向滚动条查看。单管与整线水锤段首次打开且无有效历史保存值时，壁厚 `e` 默认填 `0.06m`，启闭时间 `Ts` 默认填 `300s`，允许压力默认填 `1.0MPa` 并显示 `101.9368m` 允许压力水头；顶部批量 `e / Ts / 允许压力` 输入框也同步显示默认值，已有有效历史值不被覆盖。钢筋混凝土管和 PCCP 管会显示 `a0` 输入入口：填写后按 `cp=1/(1+0.95a0)` 折算，未填时按 `cp=1` 简化计算并提示。整线水锤段按“下游末端阀门线性启闭”展示，不再显示“代表行”或单个 `D / v0 / E / 表3水位` 输入框，而是用成员参数表展示每个成员的行范围、桩号范围、`Q / D / 管材 / E / v0 / a0`；整线级输入区只保留 `L / 统一壁厚 e / 启闭时间 Ts / 允许压力`。整线分布验算的基础采样间距为 `5m`，并强制保留起终点、管段分界点、纵断面折点和表3水位点；展开明细和 Excel 明细都必须带出所属成员、`Hst / Hmax / Hmin / D / v0 / a / cp / a0 / 管底最大压力水头 / 允许压力水头 / 承压余量 / 管顶最低压力水头 / 控制来源`，方便追溯每个采样点使用的参数。界面文案必须避免把采样点描述成独立阀门位置：采样点只用于校核同一次末端启闭水击事件对沿线各处的影响。若单管或整线满足 `Ts >= 40L/a` 免验算条件，卡片状态显示“可不验算”，数值区显示 `-`，不生成采样明细。水锤区顶部提供“导出Excel”和“验算原理”按钮：Excel 导出当前窗口全部水锤段汇总，并为已验算段生成分段明细工作表；汇总表包含允许压力、允许压力水头、承压基准和承压超限点数；免验算段在汇总表中标为“已处理 / 可不验算”，不生成明细 Sheet；未验算段只在汇总表中标记，不在导出时自动计算；导出成功后弹窗提供“打开 / 关闭”，用户可直接用默认程序打开生成的 Excel 文件。“验算原理”弹出结构化窗口，展示 GB/T 波速、图1-3-3水击类型、`Hmax/Hmin`、管底承压和管顶负压校核，并在已有结果时显示控制采样点代入值；正常界面只显示渲染后的公式，不额外显示 LaTeX 源码。
