...
| V2.11.3 | 2026-04-07 | **连续承压 xx渠 导出放宽**：连续承压整线的 `xx渠` 纵断面现在复用 `xx管` 固定 5 项表头；普通有压段第 1 行优先显示用户名称；当纵断面轴线 DXF 缺失或覆盖不全时，TXT / DXF / 合并 DXF 继续允许导出，第 4 行中心高程留空，并统一弹出“回表3导入/补全纵断面轴线 DXF 后重导”的提示。 |
