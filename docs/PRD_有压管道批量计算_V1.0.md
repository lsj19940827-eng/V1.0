...
6. 普通有压段继续使用 DXF 裁切后的纵断面；隧洞参数统一只认表1手填或 Excel 导入的现有值，弹窗只做只读摘要和缺项提示，不再新增弹窗录入口；圆拱直墙型隧洞只认 `B`，不再使用 `H`
...
- 暗涵家族不进入这条 `xx管` 隧洞摘要链：`暗涵-矩形` 兼容旧 `矩形暗涵 / 矩形暗渠 / 暗渠`，`暗涵-圆拱直墙型` 可复用圆拱几何，但表1/表3参数口径为 `B / H_total / theta_deg`，并按暗涵规则处理。
