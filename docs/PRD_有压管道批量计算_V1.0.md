...
| V2.12.13 | 2026-04-14 | **表3 锁定按钮回滚修正**：撤回“灰色但可点”的自定义样式方案，表3断面结果未就绪时，`插入渐变段 / 倒虹吸水力计算 / 有压管道水力计算 / 执行计算` 4 个主按钮统一恢复为原生灰色禁用态且不可点击；解锁后恢复原生主按钮外观，锁定原因只通过表3现有状态栏文案表达。 |
| V2.12.12 | 2026-04-14 | **表3 锁定按钮外观统一**：表3断面结果未就绪时，`插入渐变段 / 倒虹吸水力计算 / 有压管道水力计算 / 执行计算` 4 个主按钮统一改为“灰色但可点”；点击后继续走现有锁定提示。这样“有压管道水力计算”不再单独保持主色，看起来与周围按钮一致。 |
