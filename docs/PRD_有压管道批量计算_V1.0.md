...
3. `xx管` 流量段继续进入“整线卡 + 隧洞只读摘要”入口；整线仍只导入 1 份 DXF，但只要求覆盖非隧洞区间
...
- `xx管` 夹带隧洞的 mixed route 仍只导入 1 份纵断面 DXF，这份 DXF 只覆盖非隧洞子段；导出时按结构拆成上下两张表：上方普通渠道表继续跟随当前 `7/扩展项` 配置并承接隧洞，下方 `xx管` 表只保留 `有压管道 / 定向钻 / 顶管`
- mixed route 的整线卡允许在同一次弹窗里连续补导入多份纵断面 DXF；后导入文件按桩号并入当前整线，不再把前一次结果整份替换掉
- mixed route 若存在多段非隧洞区间，第一份 DXF 默认对齐首段非隧洞起点；后续补导入则默认对齐“当前第一个未覆盖非隧洞目标所在段”的起点，避免第二份文件再次贴回前半段
