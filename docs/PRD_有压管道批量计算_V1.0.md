...
- `routes[route_key].water_hammer_segments` 保存整线级水击验算段，包含稳定 `segment_key`、成员键、代表行、当前输入、正水击/负水击验算结果和更新时间；这些结果只用于弹窗内独立校核，不参与表3第38列或水位递推
...
水锤专用窗口内的“验算/刷新”采用橙色强调按钮，便于用户识别需要主动执行的校核动作；验算结果区按“数值紧凑网格 + 长结论整行换行”展示，`控制类型 / 图1-3-3对照` 等长文本不得撑宽窗口，也不依赖横向滚动条查看。单管与整线水锤段首次打开且无有效历史保存值时，壁厚 `e` 默认填 `0.06m`，启闭时间 `Ts` 默认填 `300s`；顶部批量 `e / Ts` 输入框也同步显示该默认值，已有有效历史值不被覆盖。整线分布验算的基础采样间距为 `5m`，并强制保留起终点、管段分界点、纵断面折点和表3水位点。水锤区顶部提供“导出Excel”和“验算原理”按钮：Excel 导出当前窗口全部水锤段汇总，并为已验算段生成分段明细工作表；未验算段只在汇总表中标记，不在导出时自动计算。“验算原理”弹出结构化窗口，展示当前程序已支持的线性启闭水锤公式、判定流程、图1-3-3展示口径，并在已有结果时显示当前段代入值。
