...
- 当前水击验算按《水力计算手册》中“简单管道、阀门线性启闭”的工程校核口径实现，不是特征线法瞬变全过程模拟。
- 对整线水锤段，系统假定水力控制装置位于连续有压线下游末端；界面里的 `Ts` 表示这个末端阀门的启闭时间。
- 单成员管段的水击相时按 `2L/a` 计算；多成员整线按各成员传播时间累加，即 `μ = 2 × Σ(Li / ai)`。
- 整线内每个成员继续使用自己的 `D / Q / v0 / 管材 / E / a`；`e` 与 `Ts` 当前仍作为整线统一输入。
- 沿线分布验算是工程安全校核：系统沿连续有压线采样，按采样点所在成员的参数、表3水位和管中心线高程检查正水击余量与负压余量。采样点不是独立阀门位置，也不表示每段都有独立阀门。
- 水击波速采用均质薄壁圆管简化式 `a = 1435 / sqrt(1 + (Ew / E) × (D / e))`。该式由手册通用式在 `K = E·e/r²`、`D = 2r` 条件下化简得到，数学上对应均质薄壁圆管假定。
- 对钢管、铸铁管、球墨铸铁管等可近似为均质薄壁圆管的管道，以上波速口径可作为当前工程校核公式；对 PCCP 管、钢筋混凝土衬砌管、埋藏式钢管、衬砌隧洞等复合受力管道，当前程序暂未按手册中不同 `K` 值分类型计算。
...
- `routes[route_key].water_hammer_segments` 保存整线级水击验算段，包含稳定 `segment_key`、成员键、整线统一输入、正水击/负水击验算结果和更新时间；这些结果只用于弹窗内独立校核，不参与表3第38列或水位递推
...
- 水击 E 自动填写先复用有压管道共用管材别名归一化：`PCCP管`、`预应力钢筒混凝土管`、`钢筋混凝土管` 均取 `20.6e9`，`PE管` 按 `HDPE管` 取值；未知管材仍留空，不默认套用其他材料。
- 连续有压段内若管材不一致，水击仍按整线生成一个验算段；界面用成员参数表列出每个成员的 `Q / D / 管材 / E / v0`，分布验算时每个成员按自己的管材 E 和流速取值，不再显示“代表行”。
- 整线级水击验算按“连续有压线末端阀门启闭”解释：`Ts` 是末端阀门启闭时间，采样点只用于沿程安全校核，不作为独立阀门或独立水击源。
...
水锤专用窗口内的“验算/刷新”采用橙色强调按钮，便于用户识别需要主动执行的校核动作；验算结果区按“数值紧凑网格 + 长结论整行换行”展示，`控制类型 / 图1-3-3对照` 等长文本不得撑宽窗口，也不依赖横向滚动条查看。单管与整线水锤段首次打开且无有效历史保存值时，壁厚 `e` 默认填 `0.06m`，启闭时间 `Ts` 默认填 `300s`；顶部批量 `e / Ts` 输入框也同步显示该默认值，已有有效历史值不被覆盖。整线水锤段按“下游末端阀门线性启闭”展示，不再显示“代表行”或单个 `D / v0 / E / 表3水位` 输入框，而是用成员参数表展示每个成员的行范围、桩号范围、`Q / D / 管材 / E / v0`；整线级输入区只保留 `L / 统一壁厚 e / 启闭时间 Ts`。整线分布验算的基础采样间距为 `5m`，并强制保留起终点、管段分界点、纵断面折点和表3水位点；展开明细和 Excel 明细都必须带出所属成员、`D / v0 / a`，方便追溯每个采样点使用的参数。界面文案必须避免把采样点描述成独立阀门位置：采样点只用于校核同一次末端启闭水击事件对沿线各处的影响。水锤区顶部提供“导出Excel”和“验算原理”按钮：Excel 导出当前窗口全部水锤段汇总，并为已验算段生成分段明细工作表；未验算段只在汇总表中标记，不在导出时自动计算；导出成功后弹窗提供“打开 / 关闭”，用户可直接用默认程序打开生成的 Excel 文件。“验算原理”弹出结构化窗口，展示当前程序已支持的线性启闭水锤公式、判定流程、图1-3-3展示口径，并在已有结果时显示控制采样点代入值；正常界面只显示渲染后的公式，不额外显示 LaTeX 源码。
