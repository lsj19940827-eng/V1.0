...
按钮在有压管道节点存在时按正常主按钮样式显示；当断面批量计算结果未就绪或已失效时，它会与表3里的 `插入渐变段 / 倒虹吸水力计算 / 执行计算` 一起统一显示为原生灰色禁用态，且不可点击；锁定原因通过表3现有状态栏文案提示用户先完成断面批量计算并同步到表3。
...
- 结果汇总窗采用三页签结构：`结果汇总 / 连续链总览 / 计算详情`。`结果汇总` 页以表格为主体，避免上百条计算记录被总览和详情文本挤压到只能显示少量行。
- `结果汇总` 页提供名称/流量段搜索和 `全部 / 成功 / 失败 / 起点` 状态筛选；筛选只影响当前查看范围，不改变最终回写范围。
- 用户点击“查看详情”或双击汇总表行时，窗口切换到 `计算详情` 页并显示对应记录的标准深度计算过程；打开窗口时默认先加载第一条记录详情。
- 底部主按钮语义固定为“应用全部成功结果并关闭”：无论当前是否处于搜索或筛选状态，回写仍按本轮全部成功结果执行，避免用户误以为只回写当前可见行。
