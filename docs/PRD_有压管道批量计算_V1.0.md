...
- `DiameterCandidate`：`D, V_press, hf_friction_km, hf_local_km, hf_total_km, h_loss_total_m, hf_friction_lower_km, hf_local_lower_km, hf_total_lower_km, h_loss_total_lower_m, flags`；后四项只在管材 `f` 存在规范下限时赋值。
...
- 输出区：推荐管径卡片 + 前5候选表 + 详细计算过程；球墨铸铁管在摘要、候选表、详细过程和 Word 计算书中均按“`f` 上限 / 下限”并列展示。
