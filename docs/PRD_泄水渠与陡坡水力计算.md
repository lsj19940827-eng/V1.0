...
> **创建日期**: 2026-05-12  
> **最后更新**: 2026-05-17
> **状态**: 第二版正式模块已实现，已完成输入栏组件体系对齐、加大流量输入对齐、自动分级流量、入口过流能力流量系数规范化修复、计算原理预览、计算原理页顶部 11 步流程总览、计算原理段落符号下标渲染、上下游衔接、掺气侧墙、水跃消能、出口整流、多流量控制、出处清洗、纵断面图图形与文字裁切修复、导出入口对齐、计算书开头固定格式对齐和代码评审修复；Excel/表3结构形式别名已统一接入泄水渠与陡坡专项内核
> **推荐定位**: 独立渠系建筑物设计模块，复用明渠断面水力工具；批量 Excel 和表3结构形式列中的 `充水渠`、`泄水渠`、`陡坡`、`泄槽`、`陡槽`、`泄水渠与陡坡` 统一作为泄水渠与陡坡专项类型处理

---

## 实现同步说明（2026-05-17，充水渠变坡链接入专项内核）

### 已实现范围

- 在批量 Excel、表1、共享结果和表3水面线的计算语境下，`充水渠`、`泄水渠`、`陡坡`、`泄槽`、`陡槽`、`泄水渠及陡坡`、`泄水渠与陡坡` 完全等价，计算口径统一归一为 `泄水渠与陡坡` 专项类型；“导出全部DXF”的纵断面“建筑物名称”行保留用户原始结构形式，不再强制显示统一名。
- 批量页只负责参数校验、结果表占位展示和共享数据注册；正式水面线由表3适配层计算，不再在批量页调用普通明渠内核。
- 表3适配层识别连续专项链；一条链不跨越其他结构、不跨越不同流量段。链内按相邻节点形成计算边，边的底坡取上游节点；底坡相同的连续 IP 点合并为同一固定底坡子段，底坡变化才切分新子段。
- 每个固定底坡子段调用一次 `quick_calculate_spillway_steep_chute()`；上一子段出口水位作为下一子段入口水位，末段出口水位接回后续普通水面线。
- 子段内中间 IP 点使用专项内核返回的沿程水面线按距离插值回填水深、水位、流速和断面参数；孤立单行专项节点不回退普通明渠，应提示缺少相邻下游节点。
- 七一 Excel 末尾 4 行导入后形成 3 个计算子段：`17→18` 取底坡倒数 `100`，`18→19` 取 `10`，`19→20` 取 `24.5`；第 19、20 行因底坡相同属于同一个底坡子段。

### 验收结果

- 目标回归：`46` 个测试通过，覆盖七一真实 Excel、批量导入、专项占位传参、连续变坡链、同坡 IP 合并、表3水面线和组合 DXF。

---

## 历史实现同步说明（2026-05-17，表3正式结构类型接入，已被上方连续链口径修订覆盖）

### 已实现范围

- `泄水渠与陡坡` 已成为表3正式结构类型，表3下拉、表1批量结构形式、共享结果和官方 Excel 模板下拉均支持该类型；用户手填 `泄水渠`、`陡坡`、`泄槽`、`陡槽` 时会归一为 `泄水渠与陡坡`。
- 表3现采用“连续专项链，不展开内部采样点”的口径：不再按同名建筑物成组；建筑物名称可为空或为 `-`，链内按相邻节点和上游底坡拆分固定底坡子段，首节点作为链入口，尾节点作为链出口，中间 IP 行用于平面几何和专项水面线插值展示。
- 链入口/出口只作为专项内部 `role` 参与计算，不写入表3“进出口判断”；纵断面 DXF 中 `-` 视为空名称占位，不显示 `-泄水渠与陡坡`，IP 行也不显示 `泄陡进 / 泄陡出`。
- 已按用户确认的修订口径写死渐变段规则：连续多个 `泄水渠与陡坡` 之间，即使建筑物名称、底坡、底宽、边坡或流量段不同，也按连续自由水面建筑物顺接，不自动插入渐变段；泄水渠组内部的进口、中间 IP、出口之间也不重复插入渐变段或重复扣减水头损失。
- 表3主链路已新增 `spillway_steep_chute_adapter.py`，负责校验同名组、读取表3通用列、按里程优先/坐标兜底计算长度、调用 `quick_calculate_spillway_steep_chute()`，并把起点、出口和中间行结果写回节点。
- 表3固定使用“已知长度求末端水深”模式；底宽 `B`、边坡系数 `m`、糙率 `n`、底坡倒数、设计流量 `Q`、坐标和里程直接参与计算，`m=0` 按矩形断面，`m>0` 按梯形断面。
- 专项内核已支持可选 `start_water_level`：表3传入入口水位时，内核用起点控制水深反推起点渠底高程；独立页面不传该字段时继续沿用原有 `start_bed_elevation` 或默认口径。
- 表3递推时，入口水位来自上游节点并扣除已有渐变段损失；出口水位取专项内核末端水位并接回主链路。专项水面线不可用、单行组缺少出口行或长度无效时直接报错，不输出假水位。
- 历史五排水库样例仍作为普通 `明渠-圆形` 数据回归，不代表本轮充水渠专项链口径；本轮正式验收以七一水库充水渠 Excel 为准。
- 表1/表2批量阶段只把 `泄水渠与陡坡` 作为占位行透传到表3，保留 `Q / n / B / m / 底坡倒数 / 坐标 / 建筑物名称 / 流量段 / 转弯半径`，不在表1阶段强行完整计算；官方 Excel 模板新增 10 个可选专项列，分别为入口宽度、堰上总水头、入口连接形式、手动流量系数、侧收缩系数、动能修正系数、掺气系数、侧墙安全超高、池深系数和整流长度系数，空白时使用独立模块默认值。
- 专项参数采用“Excel 预填 + 表3详情补改/查看结果”的混合口径：Excel 值导入后保存到 `section_params["spillway_steep_chute"]["advanced_params"]`，同名组推荐填在进口行；若进口行为空则取组内第一个非空值，同组多行同一参数出现不同非空值时直接阻止计算并提示冲突。
- 表3右键菜单已提供“编辑泄水渠与陡坡参数/结果”入口，可修改上述 10 个常用专项参数并写回同名组隐藏参数；计算后同一入口展示长度、入口/出口水位、总水位降、设计流量、断面参数、水面线型、末端水深、当前行流速、沿程采样点数量和风险提示。
- `.qxproj` 保存恢复继续依托 `ChannelNode.section_params`，`section_params["spillway_steep_chute"]` 保存输入、结果、`profile_points` 和风险提示；内部采样点只作为详情数据，不变成表3正式节点。

### 验收结果

- 表3专项接入、Excel 专项列和模板聚焦回归：`44` 个测试通过。
- 表3/批量大回归：`336` 个测试通过，均为业务断言通过；仅第三方 `ezdxf/pyparsing` 弃用警告。
- 泄水渠与陡坡独立专项内核回归：`27` 个测试通过。
- 表3、批量和泄水渠专项组合回归：`71` 个测试通过。
- 五排水库充水渠真实 Excel 与“导出全部DXF”回归：聚焦测试 `3` 个通过；组合 DXF 相关回归 `30` 个通过。
...
- 入口过流能力校核已按 GB 50288-2018 附录 N 修正流量系数来源：新建工况默认采用“扭曲面连接”自动计算，专业模式可选择“手动输入流量系数”；旧版隐藏默认 \(\mu=0.42\) 不再参与入口校核。
- 用户可见界面、图题、图例、空状态、表头、按钮、提示、导出章节已改为中文；输入栏补齐“清空”按钮，导出入口统一为“导出计算书”和“导出表格”，公式页和结果页中的核心公式统一通过现有公式渲染器输出。
- 输入结构已扩展到第二版：设计流量、加大流量、自动分级流量、水面线模式、起点控制水深、水面线动能修正系数、上游衔接、下游尾水、材料允许流速、掺气系数、安全超高、池深系数和出口整流参数。
...
- 计算内核已保留第一版入口 `quick_calculate_spillway_steep_chute(...)`，并扩展返回上游衔接、下游水跃、消力池、掺气侧墙、多流量控制、表3正式成组接入和计算原理所需的结构化结果。
...
- 已把本模块作为表3正式结构类型接入，但不把内部沿程 `profile_points` 展开为正式表3节点，也不自动生成整线纵断面 DXF。
...
- 计算原理页应先展示一段 11 步流程总览，用“先算什么、得到什么、给下一步提供什么”的新手口径串起完整顺序；详细公式卡片继续放在总览之后。
...
| \(b\) | 矩形底宽或梯形底宽 | m |
| \(b_c\) | 当前矩形跌口或等底宽陡坡入口控制宽度 | m |
| \(b_d\) | 陡坡底宽 | m |
| \(m\) | 梯形边坡系数 | - |
| \(\mu\) | 入口流量系数，按连接形式自动计算或由专业用户手动输入 | - |
| \(\varepsilon\) | 边界收缩系数 | - |
| \(H_0\) | 计入堰前流速水头的堰上水头 | m |
| \(n\) | 糙率 | - |
| \(i\) | 陡槽底坡 | - |
...
| \(L_r\) | 出口整流段或级间整流段长度 | m |
| \(F_b\) | 安全超高 | m |
| \(\lambda\) | 当前平底池简化计算使用的池深系数 | - |
| \(\sigma_0\) | 水跃淹没系数，保留到挖深式或更复杂消能精算 | - |
...
| 跌口形式 | 枚举 | 否 | 矩形 | 当前仅作为矩形跌口或等底宽陡坡入口校核；台堰形、梯形和复式跌口保留到后续增强 |
| 跌口宽度 \(b_c\) | 数值 | 条件必填 | 同陡槽底宽 | 用于宽顶堰过流能力校核 |
| 堰上总水头 \(H_0\) | 数值 | 条件必填 | 空 | 当前由用户直接输入，程序本次不自动由堰前水深和流速水头推导 |
| 堰前平均流速 \(V_1\) | 数值 | 否 | 后续启用 | 后续用于自动推导 \(H_0\)，当前不作为入口校核必填项 |
...
| 水面线动能修正系数 \(\alpha_e\) / `alpha_profile` | 数值 | 否 | 1.1 | 非均匀流水面线断面比能、逐段距离和沿程点统一使用，专著算例常用 \(1.05\sim1.1\) |
| 消能动能修正系数 \(\alpha_j\) | 数值 | 否 | 1.05 | 水跃和消力池使用 |
| 流速系数 \(\varphi\) | 数值 | 否 | 0.95 | 第二版消能计算使用 |
| 池深系数 \(\lambda\) | 数值 | 否 | 1.10 | 当前平底池简化计算使用 |
| 水跃淹没系数 \(\sigma_0\) | 数值 | 否 | 1.05 | 保留到挖深式或更复杂消能精算；当前平底池简化计算不使用 |
...
当前内核必须使用 `alpha_profile` 作为 \(\alpha_e\) 的正式输入字段；兼容读取 `profile_energy_alpha` 和 `alpha_e`，但新建项目和保存项目统一写回 `alpha_profile`。旧项目缺少该字段时按 \(\alpha_e=1.1\) 迁移，并提示“本版本已按计算原理启用水面线动能修正系数 alpha_e=1.1，旧版实际未使用该系数，重算结果可能与旧版略有差异。”
...
GB 50288-2018 要求跌水或陡坡的过水能力按确定的跌口断面采用宽顶堰公式计算。当前程序仅实现矩形跌口或等底宽陡坡入口的基础校核，避免把梯形、台堰形或复式跌口误表达为已实现能力。
...
当前程序由用户直接输入 \(H_0\)，本次不自动由 \(h_1+\alpha V_1^2/(2g)\) 推导。若后续启用堰前水深和堰前平均流速自动推导，必须在计算原理和导出中写清楚 \(h_1\)、\(V_1\)、\(\alpha\) 的来源。

流量系数必须有明确来源。新建工况默认采用“扭曲面连接”，程序按 GB 50288-2018 附录 N 根据入口连接形式自动计算；专业模式可改选“手动输入流量系数”，用于模型试验、既有资料复核或特殊工程。规范附录 N 给出以下常用口径：
...
该取值表示按无明显边界收缩或未另行折减处理；后续如需处理复杂边界收缩，可在专业模式扩展输入。程序不得再把 \(\mu=0.42\) 作为隐藏兜底；当既没有入口连接形式、也没有手动流量系数时，应提示“入口流量系数缺失”，并暂不强行校核入口过流能力。

旧项目兼容规则：

- 旧项目未保存入口连接形式时，首次计算按“扭曲面连接”迁移。
- 旧项目若已保存手动流量系数，则手动值优先。
- 结果风险提示中说明：“本版本已按 GB 50288-2018 附录 N 自动计算入口流量系数，旧版隐藏默认 \(0.42\) 不再使用。”

梯形跌口、台堰形跌口和复式跌口保留到后续专项增强。后续若实现梯形跌口，可按下式扩展，并必须在计算原理和导出中展示有效宽度来源：
...
当前程序使用池深系数 \(\lambda\) 进行平底池简化计算，默认 \(\lambda=1.10\)。水跃淹没系数 \(\sigma_0\) 不在当前简化池深公式中使用，保留到挖深式或更复杂消能精算。
...
class ChuteInputData:
    ...
    inlet_total_head: float | None = None
    upstream_weir_depth: float | None = None  # 后续用于自动推导 H0
    upstream_weir_velocity: float | None = None  # 后续用于自动推导 H0
    ...
    pool_depth_factor: float = 1.10
    outlet_rectification_factor: float = 10.0
...
- 表3中 `泄水渠与陡坡` 作为专项结构出现，连续同类节点形成专项链，不要求同名进出口成组，也不占用表3“进出口判断”显示口径。
- 表3递推时由上游水位确定链入口水位，适配层按底坡变化拆成固定底坡子段，并逐段调用专项内核计算出口水位、出口水深、流速、风险提示和消能结果。
- 同坡 IP 行合并到同一子段；中间 IP 行只做插值展示，不重复扣减水头损失。
- 右键菜单可打开“泄水渠与陡坡参数/结果”详情。
- 陡坡出口水位作为下游渠道递推的边界。
...
| 侧墙安全超高 | 第二版默认 \(0.4m\) |
| 池深系数 | 第二版平底池简化计算默认 \(1.10\) |
| 出口整流长度系数 | 第二版默认 \(10.0\) |
| 水跃淹没系数 | 当前不启用，保留到挖深式或更复杂消能精算 |
...
| 堰上总水头 \(H_0\) | 当前由用户直接输入；没有可靠总水头时可先跳过过流能力精算，只保留提示 | GB 50288-2018 附录 N |
...
专项模块向推求水面线提供一个结构化结果对象；表3正式接入时由适配层把连续专项链拆成固定底坡子段，并把每个子段转换为该对象的输入与输出：
...
- `warnings` 用于结果汇总和报告。
- 不把每个 `profile_point` 变成表3正式节点；表3只保留连续专项链的入口、出口和必要 IP 行。
- 表3保存时把输入、结果、`profile_points` 和风险提示落到 `section_params["spillway_steep_chute"]`，用于项目恢复、详情查看和计算过程导出。
