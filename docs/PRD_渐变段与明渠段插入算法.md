...
- 渐变段长度当前已支持项目级“组合规则”和节点级“单条覆盖”。实际采用顺序为：`单条覆盖 > 组合规则 > 公式/规范值`；若最终长度因物理可用长度不足被压缩，则额外记录 `distance_clamped=True` 告警。
- 组合规则按 `上游结构类型 + 下游结构类型 + transition_type(进口/出口)` 匹配，规则模式支持 `formula / step_up / fixed`。规则候选只来自当前工程已出现的结构组合。
- “长度规则”当前已改为**插入渐变段后的后置操作**：若表3尚无渐变段实例，点击按钮只弹出“请先插入渐变段”的引导，不再允许编辑预测组合。
- 规则对话框当前只展示表3中**已插入的真实渐变段实例组合**，命中口径统一为“当前命中 X 条”，不再展示“预计命中”。
...
- 后置长度规则的 `physical_limit` 口径已与插入渐变段阶段对齐：在“单个渐变段直接夹在两个真实节点之间”且中间无自动连接段时，上限按 `下游真实节点.station_MC - 上游真实节点.station_MC` 计算，而不是退化为当前渐变段长度。
- `physical_limit` 当前与插入阶段统一使用 `station_MC` gap 口径，而不是 `station_BC - station_EC` 的净直线段口径；后者仅可作为未来更严格几何约束的候选方案，当前实现未采用。
...
- 暗涵家族标准类型为 `暗涵-矩形` 与 `暗涵-圆拱直墙型`；旧 `矩形暗涵 / 矩形暗渠 / 暗渠` 继续作为 `暗涵-矩形` 兼容别名。
- 渐变段判断从“矩形暗涵 ↔ 明渠”升级为“暗涵 ↔ 明渠”。`暗涵-圆拱直墙型` 的水面宽可复用圆拱直墙型几何，但长度约束仍按暗涵“仅基础公式”执行，不走隧洞 `max(5h, 3D)` 约束。
- donor/自动补段当前仍只对 `暗涵-矩形` 开放，因为它具备既有 `B / H` 重算策略；`暗涵-圆拱直墙型` 需要完整 `B / H_total / theta_deg` 才能作为 donor，不得静默降级成矩形。
...
不在此集合中的结构类型直接返回 `False`（不需要渐变段）。旧 `矩形暗涵 / 矩形暗渠 / 暗渠` 先归一为 `暗涵-矩形` 后再参与判断。

#### 规则 7 细化（暗涵 ↔ 明渠）

| 明渠子类型 | 与暗涵底宽关系 | 是否插入渐变段 |
|-----------|-----------------|--------------|
| 矩形明渠 | 底宽相同 | **不插入** |
| 矩形明渠 | 底宽不同 | 插入 |
| 梯形/圆形/U形明渠 | 任意 | **总是插入** |
...
该“闸穿透”原则同样适用于倒虹吸相邻流速提取链路（`v₁/v₃` 与 `v₁加大/v₃加大` 的自动读取）。
...
| 暗涵-矩形 / 暗涵-圆拱直墙型 | 仅基础公式 | 仅基础公式 | 暗涵家族约束；旧 `矩形暗涵` 兼容到 `暗涵-矩形` |
...
| 倒虹吸/有压管道 | `5h` | `6h` |
| 暗涵-矩形 / 暗涵-圆拱直墙型 | `L_basic` | `L_basic` |
...
```python
{
    ...
    "requested_length",       # 规则目标长度（修约后/固定值）
    "physical_limit",         # 当前拓扑下允许的物理上限
...
2. 若命中项目级组合规则：
   - `formula`：直接采用公式值
   - `step_up`：先按步长向上修约得到 `requested_length`
   - `fixed`：采用固定值
3. 若该条渐变段存在 `transition_length_override_m`，则覆盖组合规则结果
4. 对组合规则与单条覆盖都计算 `physical_limit`
5. 若 `requested_length / override_value` 超过 `physical_limit`：
   - 组合规则：自动裁剪到 `physical_limit`，并记录 `distance_clamped=True`
   - 单条覆盖：直接拒绝保存，并提示用户重新插入渐变段或调整结构布置
6. 最终采用值写入 `actual_length`

补充口径：

- 允许单条覆盖值或固定值小于 `formula_length`，但会在详情弹窗、tooltip 和表格着色中显示警告。
- `step_up` 的修约目标始终是公式/规范值，而不是基础公式 `L_basic`。
- 当前规则对话框只面向已插入实例；“是否需要补段”的插入判断不再依赖插入前可编辑的预测规则表。
- 当前 `physical_limit` 的基础口径与插入阶段一致，优先取包围该渐变段的前后真实节点 `station_MC` 差值；若该渐变段相邻存在连续自动连接段，则可叠加这些连接段的 `stat_length` 作为可吞并余量。
- 因此，当前实现下的“物理极限”不是简单的 `当前节点.station_BC - 上一真实节点.station_EC`。
...
**视觉区分**：
- 文字颜色：绿色（`#2E7D32`）
- 悬浮提示："自动插入的明渠连接段，用于计算两个建筑物之间的沿程摩阻及弯道附加水头损失。几何列留空因为该行不是真实IP转折点。"
...
补充显示规则（2026-05-11）：
- 原生IP节点的三项复核长度按表格三位小数显示值判断；显示后仍小于 0 的单元格仅数字标红（`#C62828`），不改变计算结果。
- 显示为 `-0.000` 的极小值视为显示层非负，不标红、不提示，避免浮点尾差误导用户。
- 点击表3“执行计算”后，如存在复核长度负数，顶部 `InfoBar.warning` 汇总行号、列名和数值；提示不阻断流程，超过 5 处时只展示前 5 处并追加“等N处”。
- 渐变段、自动插入明渠和倒虹吸行继续留空，不参与复核长度负数提示。
...
**IP 列显示规则**（`get_ip_str()`）：
- 系统内部继续保留原始 `ip_number`，并额外生成面向界面与导出的 `display_ip_number`
- 隧洞/渡槽/倒虹吸/有压管道/定向钻/顶管的进出口：仅显示 `{建筑物名称}{类型标签}{进/出}`，如 `油房坨隧进`、`白果湾有压管道出`、`穿路段定向钻进`
  - 类型标签：隧洞→"隧"，渡槽→"渡"，倒虹吸→"倒"，有压管道→"有压管道"，定向钻→"定向钻"，顶管→"顶管"
  - 这些进出口行**不占显示用 IP 编号**，后续普通点与建筑物内部真实转点继续按 `IP0 / IP1 / IP2 ...` 顺排
- **矩形暗涵**：继续仅显示 `IP{n}`，**不加进/出后缀**，并沿用原有编号显示规则
...
- 规则对话框只展示“当前工程已经插入到表3中的真实渐变段组合”，不做全量排列。
- 若当前表3还没有渐变段实例，点击“长度规则”只弹出引导，不进入可编辑规则表。
...
- 该对话框仅在**已插入渐变段**后开放；未插入时弹出“先插入渐变段”的说明对话框。
- 命中列统一展示“当前命中 X 条”。
- 对话框底部固定提示：向上修整先计算目标整数长度，再按当前物理可用长度裁剪；若当前拓扑不足，最终长度可能不是整数。
- 应用完成后的成功提示需区分：
  - `已更新 N 条渐变段`
  - `其中 M 条受物理极限约束，未能修整到目标整数`
...
| 双击列 | 弹出内容 |
|--------|---------|
| 渐变段长度L | 基本公式+各约束条件+规则目标长度+物理上限+当前采用值+来源+警告；弹窗底部可保存单条覆盖或恢复公式/规则 |
...
| 管身段 $\Delta Z_2$ | $h_f + h_j + \frac{v^2 - v_2^2}{2g}$（沿程+局部+速度水头差） | 不含进出口渐变段 |
...
2. **同流量段 donor 搜索**
    - 先从倒虹吸上游侧按表格行序逆向扫描同段可用邻接断面，再从下游侧按表格行序正向扫描同段可用邻接断面。当前可用 donor 范围为“明渠 + 暗渠（`暗涵-矩形` / 旧 `矩形暗涵`）”；隧洞不纳入 donor，`暗涵-圆拱直墙型` 待完整参数链落地后再开放 donor。
...
3. **跨流量段 donor 搜索与重算**
    - 若同流量段内无可用 donor，则继续按“跨段上游优先、跨段下游次之”扫描其他流量段的可用 donor。当前 donor 范围同样为明渠或暗渠（`暗涵-矩形` / 旧 `矩形暗涵`）。
...
4. **跨段重设计策略**
   ...
   - `明渠-U形`：复用 donor 的 `alpha_deg / theta_deg / n / slope_inv`，自动搜索最小可行 `R`。  
    - `暗涵-矩形`（含旧 `矩形暗涵`）：先按 donor 原 `B/H` 试算；若原尺寸不足，则优先保留原底宽 `B`，自动增大总高 `H` 后重算；若仍失败，再继续尝试后续 donor。`暗涵-圆拱直墙型` 不得套用这条矩形重算策略。
...
6. **当前实现同步说明（2026-03-16）**
   ...
   - `催龙村` 类场景仍优先命中同流量段 donor，但在新口径下整座倒虹吸统一采用同一 donor 方向，不再按 `v₁` / `v₃` 分别套用不同优先级。
    - 当 Excel/批量结果仅提供 `暗渠` 或 `矩形暗渠` 时，当前口径会先在导入阶段把名称归一为标准类型 `暗涵-矩形`，旧内部值 `矩形暗涵` 仅作兼容别名，随后纳入 donor 搜索与 UI 展示链路；前台提示文案统一使用“邻接断面”“暗渠/矩形暗渠”口径。
...
| **v3.4** | **2026-04-14** | **暗涵家族口径同步**：明确 `暗涵-矩形` / `暗涵-圆拱直墙型`、旧名兼容、`暗涵-圆拱直墙型` 复用圆拱水面宽但执行暗涵基础长度约束；donor 当前仍只开放 `暗涵-矩形`，圆拱直墙型不得退化成矩形 donor。 |
| **v3.5** | **2026-05-11** | **复核长度提示同步**：表3三项复核长度显示为负数时仅数字标红；执行计算后用一次非阻断 InfoBar 汇总行号、列名和数值，超过 5 处时截断汇总；显示为 `-0.000` 时不提示。 |
