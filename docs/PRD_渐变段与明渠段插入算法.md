...
倒虹吸参数自动导入时，流速读取遵循以下规则（与 2026-03-09 代码实现一致）：

1. **同方向、同流量段扫描（主链路）**  
   - 进口侧：从倒虹吸进口向上游（`step=-1`）扫描；出口侧：从倒虹吸出口向下游（`step=+1`）扫描。  
   - 仅在**同一 `flow_section`** 内寻找候选明渠节点。
2. **闸穿透**  
   - 扫描过程中跳过闸类节点（分水闸/分水口/泄水闸/节制闸），不在闸处提前终止。  
   - 同时跳过渐变段、倒虹吸节点、自动插入连接段。
3. **不跨流量段（硬约束）**  
   - 扫描一旦遇到不同 `flow_section`，立即停止，不跨段取值。
4. **同段最近明渠兜底**  
   - 若同方向扫描到同段边界仍无候选，回退到“同流量段内距离该倒虹吸最近的明渠节点”。  
   - 一次性取该节点的 `v` 与 `v加大` 作为同来源兜底，并标记来源为 `same_section_nearest_channel_fallback`。
5. **同段无明渠则保持缺失并告警**  
   - 若同流量段内不存在可用明渠节点，则保持缺失（来源标记 `missing`），不做跨段借值与公式估算补值。  
   - UI 需提示用户人工核对并补录。
