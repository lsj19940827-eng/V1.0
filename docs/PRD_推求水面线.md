...
> 版本：v1.5 | 创建：2026-02-26 | 最后更新：2026-03-13 | 状态：已实现
...
2. 通过 `SiphonDataExtractor` 从节点表提取倒虹吸分组
   ...
   - `v₁/v₃` 与 `v₁加大/v₃加大` 当前实现采用统一 donor 顺序：`同流量段上游 -> 同流量段下游 -> 跨流量段上游 -> 跨流量段下游`。跨段 donor 允许使用倒虹吸目标流量重算，并在必要时按 donor 型式自动重设计；详细规则见 `PRD_渐变段与明渠段插入算法.md` §12.6。
   - 当前实现同步说明（2026-03-13）：`MultiSiphonDialog` 在窗口初始化后仅弹出 1 条“流速来源提醒”中文汇总提示，不再按类别连续弹出多条告警；提示内部按“同段借用”“跨段借用并重算”“借用后重新确定断面尺寸”“仍未找到可借用断面”分段展示，并使用“借用断面”“借自第X流量段”“按当前流量段的设计流量和加大流量重新计算”等中文表述，便于用户直接核对。
