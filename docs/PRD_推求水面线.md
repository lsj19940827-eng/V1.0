...
> 版本：v1.6 | 创建：2026-02-26 | 最后更新：2026-03-16 | 状态：已实现

---

## 当前实现同步说明（2026-03-16）

- 推求水面线的渐变段插入链路已支持从 `明渠 + 矩形暗涵` 自动借用邻接断面参数，不再要求同流量段必须存在明渠。
- 插入补段时，矩形暗涵会以内部标准类型 `矩形暗涵` 创建节点，并写入 `section_params["B"]`、`section_params["H_total"]`、`structure_height`、`water_depth`、`roughness`、`slope_i` 等字段，供后续几何、水力和统计链路继续使用。
- donor 首选家族规则为：仅当空隙两端真实结构都属于矩形暗涵场景时首选暗渠，否则首选明渠；整体排序采用 `同段首选 -> 同段非首选 -> 跨段首选 -> 跨段非首选`。
- 前台补段对话框已允许 `矩形暗涵` 作为结构形式，明渠显示 `B / m / n / 1/i / Q`，矩形暗涵显示 `B / H / n / 1/i / Q`，无 donor 的位置保留人工填写，不再走几何兜底估算。
- 批量补段弹窗当前实现已增加“状态/来源”列，用于显式区分自动推荐来自同段/跨段、明渠/暗渠，或该行需手动填写。
- `available_length` 当前实现采用统一的“无可用长度”语义：长度不足时记为 `0.0`，不再向 UI 或测试暴露负值差额。
...
| **插入渐变段** | PrimaryPushButton | 调用 `_insert_transitions()` 自动插入渐变段/补段行 |
...
| 导入Excel | PushButton | 从 Excel 文件导入节点数据；识别列名（模糊匹配）：流量段、建筑物名称、结构形式、X/E坐标、Y/N坐标、转弯半径、底宽B、直径D、半径R、边坡m、糙率n、底坡1/i、流量Q、水深h、流速v；旧版简写"梯形/矩形/圆形"自动映射为"明渠-×"；`暗渠`、`矩形暗渠` 自动归一为内部标准类型 `矩形暗涵` |
...
| **生成断面汇总表** | 明渠/隧洞/渡槽/暗涵/倒虹吸/有压管道等各断面尺寸及水力要素汇总 | DXF |
| **IP坐标及弯道参数表** | IP点坐标、桩号、转角、半径、切线长、弧长、底高程 | DXF + Excel |
| **导出全部DXF** | 以上三表合并到同一DXF文件，分图层管理；导出前弹出倒虹吸/有压管道材质与DN设置 | DXF |
...
- “导出全部DXF”和“生成断面汇总表”共用同一套有压流参数链路，参数配置粒度为“建筑物名称 + 流量段”。
- 参数对话框中，倒虹吸/有压管道按实际实体逐条显示，不在对话框内预合并；名称直接显示为 `建筑物名称-流量段`。若建筑物名称为空，则回退为 `结构类型-流量段`。
- 倒虹吸/有压管道断面汇总只基于“实际建筑物 + 实际流量段”的映射生成，不允许将某建筑物的 DN / 材质与其他流量段做笛卡尔组合。
- 同一流量段内的去重签名为 `Q`、`n`、`DN_mm`、`pipe_material`：
  - 该流量段只有 1 座建筑物时，汇总行命名为 `建筑物名称-流量段`。
  - 该流量段有多座建筑物且参数全部相同时，合并后只保留 1 行，名称显示为纯 `流量段`。
  - 该流量段最终保留多行时，所有保留行都使用 `建筑物名称-流量段` 命名，并保持首次出现顺序。
...
- `cad_tools.py` 当前实现会先从节点中提取倒虹吸/有压管道的 `name`、`flow_section`、`display_name`、默认 `DN_mm` 和 `structure_kind`，再与用户缓存参数做合并。
- `custom_pressurized_pipe_params` 的项目缓存格式已升级为对象列表，字段包含 `name / flow_section / display_name / pipe_material / DN_mm / structure_kind`；旧三元组格式 `(name, material, dn)` 读取时会自动迁移。
- 旧缓存迁移规则如下：
  - 若旧缓存名称只匹配到 1 个实际流量段，则直接迁移到该条实体行。
  - 若同名建筑物跨多个流量段，则按名称复制到所有匹配的实体行。
  - 若已存在精确的“名称 + 流量段 + 结构类型”缓存，则优先使用精确命中值。
- 若某倒虹吸/有压管道缺少可解析的 `flow_section`，参数对话框会明确提示该条目被跳过，本次配置和导出均不为其生成汇总行。
...
2. 通过 `SiphonDataExtractor` 从节点表提取倒虹吸分组
   ...
   - `v₁/v₃` 与 `v₁加大/v₃加大` 当前实现采用统一 donor 顺序：`同流量段上游 -> 同流量段下游 -> 跨流量段上游 -> 跨流量段下游`。donor 候选范围已从“仅明渠”扩展为“明渠或暗渠（当前落地为矩形暗涵）”，隧洞不纳入 donor；详细规则见 `PRD_渐变段与明渠段插入算法.md` §12.6。
   - 当前实现同步说明（2026-03-16）：同流量段命中 donor 后直接复用其 `velocity / velocity_increased` 与邻接断面参数；跨流量段明渠仍沿用“原尺寸试算 + 必要时自动重设计”，跨流量段暗渠则先按原 `B/H` 试算，失败后保留原底宽 `B` 自动增大高度 `H` 后重算，若仍失败则保持 `missing` 并要求人工确认，不自动编造默认流速。
   - 当前实现同步说明（2026-03-16）：`MultiSiphonDialog` 在窗口初始化后仅弹出 1 条“流速来源提醒”汇总提示，文案已统一升级为“邻接断面流速”口径，并区分“同段复用”“跨段借型并重算”“借用后重定邻接断面”“仍未找到可用邻接断面”；提示中会展示 donor 类型（明渠/暗渠）、来源流量段、扫描方向与重设计方式，便于用户直接核对。
...
| v1.5 | 2026-03-13 | **代码**：倒虹吸 donor 提取改为整座倒虹吸统一上游优先；候选顺序调整为 `同流量段上游 -> 同流量段下游 -> 跨流量段上游 -> 跨流量段下游`；跨流量段 donor 支持按目标流量重算并在必要时自动重设计（圆形/梯形/矩形/U形）；`SiphonGroup` 新增 per-side provenance，`MultiSiphonDialog` 新增同段 donor / 跨段 donor / donor 重设计 / 缺失四类汇总提示。**PRD修订**：补充 §9.2 中 `SiphonDataExtractor` 的当前实现说明，并与 `PRD_渐变段与明渠段插入算法.md` §12.6 对齐。 |
| v1.6 | 2026-03-16 | **代码**：倒虹吸 donor 候选扩展为“明渠 + 暗渠（矩形暗涵）”；Excel/批量结果兼容 `暗渠`、`矩形暗渠` 并统一归一为内部 `矩形暗涵`；跨流量段暗渠新增“原 B/H 试算，失败后保留 B 自动增大 H”重算策略；缺失时仍保持人工确认，不自动填默认流速；UI 文案统一升级为“邻接断面流速/邻接断面参数”，并在汇总提示中展示 donor 类型与重设计方式。**PRD修订**：同步更新 §6.4 Excel 导入口径、§9.2 当前实现说明，并与 `PRD_渐变段与明渠段插入算法.md` §12.6 对齐。 |
