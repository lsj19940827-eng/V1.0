...
- 多工况断面图每个成功工况一个小图，设计水位保留蓝色实线和水体填充；启用加大流量且加大水深有效时，在同图叠加“加大水位”虚线和 `h加大=x.xxm` 竖向双向水深尺寸标注。
...
- 多工况仍保持每个成功工况一个小图；本次不改变计算公式、输入表单、DXF 或 Word 导出。
...
- **多工况**: 每个成功工况一个小图，图内叠加设计水位和加大水位；水体填充仍表示设计水位
...
- **运行方式**: `python tests/test_tunnel_kernel.py`；反推助手补充运行 `pytest tests/test_tunnel_clearance_sizing_unit.py tests/test_tunnel_clearance_sizing_panel_unit.py tests/test_tunnel_clearance_sizing_dialog_unit.py`；工况对比补充运行 `pytest tests/test_tunnel_comparison_unit.py tests/test_tunnel_dxf_export_unit.py tests/test_multi_case_dxf_export_unit.py`
