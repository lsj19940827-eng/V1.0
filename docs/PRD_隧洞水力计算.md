...
- 汇总内容包含设计/加大流量下的水深、流速、净空高度、净空比例，以及圆形直径、平底圆形平底宽和总高、圆拱直墙型底宽/总高/直墙高度/拱部参数、马蹄形半径或等效直径等关键尺寸。
...
- 结构表保留隧洞关键尺寸：`底宽 B`、`直径 D`、`半径 r`、`洞总高 H`、`直墙高度 H直`、`洞身周长`、`洞身断面积`，其中周长和面积继续使用完整洞身几何口径。
...
- 结构尺寸表补齐 `等效直径 2r`、`拱顶圆心角 θ`、`拱半径 R拱`、`拱高 H拱` 等重点结构派生值；不适用的结构形式对应单元格留空。
- 删除原“控制余量类型 / 控制余量”摘要列，工况对比只保留设计/加大净空高度和净空比例等明确重点结果列。

### 1.13 当前实现同步（2026-05-01，断面图水位线）

- 单工况断面图仅在启用加大流量且 `h_increased` 有效时显示加大流量子图；未启用时只显示设计流量图。
...
matplotlib 绘制:
- **单工况**: 默认显示设计流量工况；启用加大流量且结果有效时，并排显示加大流量工况
- **多工况**: 每个成功工况一个小图，图内叠加设计水位和加大水位；水体填充仍表示设计水位
- 多工况叠加加大水位时，同步显示 `h加大=x.xxm` 竖向双向尺寸箭头
