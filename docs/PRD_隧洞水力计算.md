...
- 结构尺寸汇总同步显示 `洞身周长`，并复用工况对比的完整洞身几何口径；该值不使用过水湿周，不随当前水深变化。
...
- 共享断面图布局默认从 360px 行高起步；明渠、渡槽、暗涵会在等比例子图过窄时动态补足画布高度，隧洞则继续使用 520px 专用行高。
...
- 明渠、渡槽、暗涵改为共享等比例可读高度校正后，隧洞仍保留 520px 行高、顶对齐和隐藏页签预渲染策略，不套用非隧洞的动态高度补偿。

### 1.17 当前实现同步（2026-05-10，四面板布局统一）

- 明渠、渡槽、隧洞、暗涵左侧输入栏统一固定为 420px，隧洞常规紧凑工况标签在标准窗口下稳定每行 3 个，第 4 个进入下一行。
- 隧洞多工况断面图继续按成功工况出图，默认每行 2 个子图，明显窄宽度才退为 1 个；9 个圆拱直墙型工况继续保持 2 列 5 行、2600px 画布高度。
- 隧洞的 520px 行高、顶对齐和关闭动态高度补偿已收口为共享布局配置，不再在面板内散落临时补丁；计算、项目保存、DXF、Word 和 TXT 规则不变。
...
| 2026-05-09 | v1.26 | 隧洞多工况断面图单独使用 520px 行高，解决圆拱直墙型等高瘦断面在等比例坐标下被压窄、右侧留出大片空白的问题；当时明渠、渡槽、暗涵仍沿用共享默认行高。 |
...
| 2026-05-09 | v1.32 | 明渠、渡槽、暗涵新增等比例可读高度校正后，隧洞继续保持 520px 专用行高、顶对齐、隐藏页签预渲染和 9 工况 2600px 目标高度，不受非隧洞策略影响。 |
| 2026-05-10 | v1.33 | 四个设计面板输入栏统一固定为 420px，常规工况标签每行 3 个；隧洞断面图改为共享配置入口表达 520px 行高、顶对齐和 9 工况 2600px 高度，继续默认 2 列、窄宽度 1 列。 |
| 2026-05-10 | v1.34 | 修复结果页“工况快捷导航”文字裁切：导航标签按自身真实宽度流式排布，`工况10` 及后续“计算失败”标签不再被压窄；计算、项目保存、DXF、Word 和 TXT 规则不变。 |
| 2026-05-10 | v1.35 | 结果页“重点结果汇总”的结构尺寸补齐 `洞身周长`，复用工况对比中的完整洞身几何口径，并在 Word 汇总表中同步显示。 |
