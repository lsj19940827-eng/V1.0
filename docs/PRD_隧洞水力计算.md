...
- 对比表固定列为：工况、断面类型、设计流量、加大流量、设计水深、设计流速、加大水深、加大流速、底宽 B、直径 D、半径 r、总高 H、直墙高度 H直、洞身周长、洞身断面积。
- 对比表同时显示设计工况与加大工况的水深、流速；未启用加大流量或无加大结果时，加大列留空。
- 洞身周长和洞身断面积固定采用完整洞身几何口径，不使用过水湿周或过水面积。
- 多工况 DXF 导出在原断面图网格下方追加「隧洞多工况参数对比表」；只有一个工况走原单工况直接导出时，仍保持原单断面 DXF 行为。
...
- **右侧**: 输出区域（QTabWidget，三个Tab）
  ...
  - Tab3「工况对比」: QTableWidget 显示已计算工况的水力结果和洞身尺寸
...
- 水深和流速分别列出设计工况与加大工况；未启用或无结果的加大列留空。
- `洞身周长` 与 `洞身断面积` 是完整洞身几何指标，不随当前水深变化。
- 表格支持 `Ctrl+A` 全选后 `Ctrl+C` 复制到 Excel；复制内容第一行带表头，列用制表符分隔，空值保持为空单元格。
...
- 隧洞多工况 DXF 在断面图网格下方追加「隧洞多工况参数对比表」，表头带单位，内容与界面「工况对比」页签同源。
- 对比表 DXF 表头单位使用 CAD MTEXT 上标控制码显示 `m³/s`、`m²`；表头按“名称 / 单位”两行显示，列宽按表头和内容自动适配。
- `export_combined_case_dxf()` 通过可选 `draw_summary_table` 回调追加附表；不传该回调时，其他模块的多工况 DXF 行为保持不变。
...
- `draw_tunnel_comparison_table()`: 隧洞多工况参数对比表绘制，复用中文字体与参数文字图层，并按内容自动适配列宽
...
- `tests/test_tunnel_comparison_unit.py` — 隧洞工况对比行、完整洞身几何指标和多工况 DXF 回调链路
...
| 17 | `test_tunnel_comparison` | 五类隧洞对比行、完整洞身周长/面积、DXF 对比表标题和列名 | ~8 |
