...
- `tools/release.py` 已收敛为正式单通道：仅允许 `master` 分支正式发布
- 发布脚本仅更新正式 Gist，不再维护测试 Gist 分流
...
7. **发布通道风险**：当前仅保留正式 Gist 单通道，需避免任何临时包误写正式更新源。
...
| C10 | 发布回滚治理 | 校验单通道发布与回滚条款 | `master` 正式发布、正式 Gist 更新、可回滚演练 | 补充第八章策略 |
...
| S8：正式更新单通道 | 仅正式 Gist 更新，不向用户暴露测试切换 |
