...
| 流量设置 | 主界面继续按当前流量段只读查看设计流量/加大流量；手工 `Q加大` 只认导入 Excel 模板第1行右侧列对，未填则回落自动比例 |
...
当前实现同步说明（2026-04-18）：
- 手工 `Q加大` 的唯一入口固定为导入 Excel 模板 `导入模板` 页第1行右侧的“标签列 + 值列”对，表3不再提供单独手工入口。
- 模板默认已在 `I1:AV1` 预铺 20 组流量段列对；这只是模板默认数量，真实解析会继续按同样格式向右扩展。
...
与现有 `INPUT_HEADERS` 完全一致（28列），冻结前4列：

| 冻结列(4) | 可滚动列(24) |
|-----------|-------------|
| 序号、流量段、建筑物名称、结构形式 | X、Y、Q(m³/s)、糙率n、比降(1/)、边坡系数m、底宽B(m)、明渠宽深比、半径R(m)、直径D(m)、矩形渡槽深宽比、倒角角度(°)、倒角底边(m)、圆心角(°)、不淤流速、不冲流速、转弯半径(m)、管材、左上坡m1、平台宽B1(m)、左下坡m2、渠底宽B2(m)、右坡m3、平台高差h1(m) |
