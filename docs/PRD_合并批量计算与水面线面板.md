...
## 当前实现同步说明（2026-05-18，七一水库充水渠真实回归）

- 本轮纠正此前“五排水库充水渠”理解偏差：真实验收样例改为 `data/七一水库充水渠批量计算用表.xlsx`。
- 七一 Excel 第 17-20 行结构形式已由 `明渠-矩形` 改为 `充水渠`；第 1 流量段手工 `Q加大` 保持空白，表示继续采用自动比例和自动加大流量；末尾 4 行补齐不淤/不冲流速 `0.1/100`，不改变坐标、流量、糙率、底宽、边坡、底坡倒数和转弯半径等核心工程数据。
- 表1“结构形式”语境下，`充水渠`、`泄水渠`、`陡坡`、`泄槽`、`陡槽`、`泄水渠及陡坡`、`泄水渠与陡坡` 完全等价，内部统一归一为 `泄水渠与陡坡`；这不改变基础信息行 `渠道级别=充水渠` 的原有含义。
- 上述名称作为占位行进入批量计算和共享结果，正式水面线由表3按连续专项链调用 `spillway_steep_chute_adapter`；批量结果的计算类型统一为 `泄水渠与陡坡`，但 `display_structure_type` 保留用户原始结构形式，DXF 纵断面“建筑物名称”行按该原始名称显示。允许建筑物名称为空或 `-`，`-` 仅表示空名称占位，不判断表3“进/出”，导出全部 DXF 不显示 `-泄水渠与陡坡` 或 `泄陡进 / 泄陡出`。
- 七一真实回归要求导入 18 行、末尾 4 行保留 `充水渠`，批量计算成功 18 行且失败 0 行；“导出全部DXF”必须能被 `ezdxf.readfile()` 打开，并包含纵断面、断面汇总表和 IP 坐标表。
- Excel 基础信息单元格如为简单数字公式（例如 `=352.44`），导入时按其数字值处理；不得因为公式缓存缺失而回退到默认起始水位。
...
当前实现同步说明（2026-05-14）：
- 官方空白 Excel 模板 `多流量段批量计算_导入Excel（模板）.xlsx` 已新增新手填写说明页：`0-先看这里`、`1-公共字段`、`2-明渠`、`3-渡槽`、`4-隧洞`、`5-暗涵`、`6-承压与闸类`、`7-字段速查`。
- `导入模板` 仍保持第一张工作表和活动工作表；第1行手工 `Q加大` 入口和当前导入解析口径不变。
- 说明页只解释每类结构形式的必填、条件必填、可选、不要填写和冲突字段；不新增计算类型，也不改变批量断面计算逻辑。
- `结构形式` 列已增加下拉提示，候选项来自当前批量面板支持的全部 `SECTION_TYPES`；现已包含“充水渠”，并兼容用户填写“泄水渠”“泄水渠与陡坡”，三者导入后统一按逐行 `充水渠` 渠道段计算。
- 批量输入表和官方模板表头已按填写心智重排：`拉杆高度(m)` 放在 `矩形渡槽深宽比` 后，`直墙高度H直(m)` 放在 `圆心角(°)` 后；旧 Excel 继续按表头映射导入，旧工程无表头顺序时按重排前 32 列顺序迁移。
- 旧版无 X/Y 且表头不可识别的 Excel 兜底导入按当前列常量映射；旧表没有的 H/B、H、拉杆高度、H直 和复式梯形字段保持空白，避免倒角、圆心角、不淤/不冲流速等列被挤偏。
- 官方模板已统一视觉样式：`导入模板` 固定为活动页，`A2:AF500` 使用统一表头、筛选、边框和整行斑马纹；说明页统一冻结在表头下方，修复重排列后个别列背景、边框和字体不一致的问题。
...
与现有 `INPUT_HEADERS` 完全一致（32列），冻结前4列：

| 冻结列(4) | 可滚动列(28) |
|-----------|-------------|
| 序号、流量段、建筑物名称、结构形式 | X、Y、Q(m³/s)、糙率n、比降(1/)、边坡系数m、底宽B(m)、暗涵高宽比H/B、暗涵高度H(m)、宽深比β、半径R(m)、直径D(m)、矩形渡槽深宽比、拉杆高度(m)、倒角角度(°)、倒角底边(m)、圆心角(°)、直墙高度H直(m)、不淤流速、不冲流速、转弯半径(m)、管材、左上坡m1、平台宽B1(m)、左下坡m2、渠底宽B2(m)、右坡m3、平台高差h1(m) |

`充水渠 / 泄水渠 / 陡坡 / 泄槽 / 陡槽 / 泄水渠及陡坡 / 泄水渠与陡坡` 使用公共列即可同步到表3：建筑物名称允许为空或 `-`，不用于表3进出口显示；`Q / 糙率n / 比降(1/) / 边坡系数m / 底宽B(m) / X / Y / 转弯半径` 作为专项链参数进入批量计算和表3递推。
