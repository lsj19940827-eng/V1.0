...
- 加大流量输入已统一为两种模式：`按比例` 与 `按Q加大`；保留原“考虑加大流量”开关，默认仍按 `按比例` 打开。
- `按比例` 模式下，比例输入允许留空；留空时继续按既有规范表自动查表，不改变老口径。
- 加大流量链路的内部计算已改为高精度直传：比例、倍率和 `Q加大` 不再在进入后续水深 / 流速计算前提前截成 3 位。
- 结果页、TXT、Word、DXF 中，加大比例统一显示为 3 位小数（如 `18.200%`），公式代入里的比例小数 / 倍率统一显示为 5 位（如 `0.18200`、`1.18200`）。
- `按Q加大` 模式下，输入的是“加大后的总流量”，不能为空，且必须严格大于设计流量 `Q`；不满足时界面直接提示，不进入计算。
- 同一时刻只保留一种正式输入：当前模式的另一项只做灰色换算提示，不作为第二个必填项。
- 首次进入面板时也必须立即按当前模式收口显示；默认只显示“按比例”输入，不得把“按Q加大”一栏一并露出。
- 工况保存新增 `inc_mode / inc_q_text`，旧工程只有 `inc_pct` 时仍按 `按比例` 回显；关闭开关只隐藏控件，不主动清空上次输入。
- 结果页、TXT、Word、DXF 会同时展示 `输入方式 / 用户输入值 / 系统换算值`；底层计算仍继续接收比例参数，不改内核函数签名。
...
- 汇总内容包含设计/加大流量下的水深、流速、渠道高度，以及对应断面特有的底宽、渠底宽、直径、半径、宽深比等关键尺寸；未启用加大流量时隐藏加大工况空值，只显示短提示。
- 明渠汇总里的“超高”只展示本次用于确定渠道高度的规范超高：启用加大流量时显示“加大渠道超高 Fb = 0.25 × h加大 + 0.2”；未启用加大流量时显示“设计渠道超高 Fb = 0.25 × h设计 + 0.2”；不显示 `H - h设计` 这种设计水面到渠顶的实际余高。
...
## 当前实现同步说明（2026-05-01，工况对比重点结果）

- 工况对比水力表同步重点结果口径，新增设计/加大渠道超高列，以及圆形明渠专用的设计/加大净空高度和净空比例列。
- 圆形明渠工况对比改用 `y_d / V_d / Q_inc / y_i / V_i / FB_d / PA_d / FB_i / PA_i` 专用结果字段，避免设计/加大水深、流速、流量和净空指标在对比表中留空。
- 删除原“控制余量类型 / 控制余量”摘要列，工况对比只保留明确的重点结果列，避免与设计/加大超高和净空指标重复。

## 当前实现同步说明（2026-05-01，断面图水位线）

- 单工况断面图统一显示一张设计流量图；勾选且加大水深有效时，在同一坐标轴叠加“加大水位”虚线和加大水深标注。
- 多工况断面图每个成功工况一个小图；设计水位保留蓝色实线和水体填充，勾选且加大水深有效时叠加“加大水位”虚线和加大水深标注。
- 多工况叠加加大水位时，同步新增从 `y=0` 到加大水深的竖向双向尺寸箭头，文字统一为 `h加大=x.xxm`，颜色跟随加大水位线，并放在既有 `h / H / R / B / B1 / B2` 标注外侧避免压盖。
- 圆形明渠的加大水位使用 `y_i / Q_inc / V_i`；其他断面使用 `h_increased / Q_increased / V_increased`；加大水深缺失、为 0 或无效时不绘制。

## 当前实现同步说明（2026-05-07，断面图绘制底座统一）

- 明渠界面断面图已改为复用 `section_shapes.py` 与 `section_plotting.py`：梯形、矩形、复式梯形、圆形、U 形只在明渠面板组织工况，轮廓、水域填充、水位线、尺寸标注、标题、网格和视图范围由共享绘图底座完成。
- 本次只统一 matplotlib 界面断面图绘制，不改变明渠计算内核、输入表单、DXF 导出和 Word 导出规则。
- 明渠特有的 `B1 / B2 / h1`、坡比、圆形水深 `y`、U 形 `R / θ` 等标注继续保留，但标注绘制入口已收口到共享绘图器。

## 当前实现同步说明（2026-05-08，单工况单图叠加）

- 明渠普通单项入口和只有 1 个成功工况的汇总入口均统一为一张断面图；设计水位保留水体填充和蓝色实线，加大水位只在同图叠加虚线、“加大水位”文字和 `h加大=x.xxm` 标注。
- 圆形明渠继续使用 `y_i / Q_inc / V_i` 作为加大水位来源；梯形、矩形、复式梯形和 U 形继续使用 `h_increased / Q_increased / V_increased`。
- 界面断面图的水平尺寸文字改用固定点数偏移，避免小图中底宽 `B` 文字压到尺寸箭头；计算、DXF 和 Word 规则不变。
- 多工况仍保持每个成功工况一个小图；本次不改变计算公式、输入表单、DXF 或 Word 导出。

## 当前实现同步说明（2026-05-01，工况标签交互）
...
| `panel.py` | PySide6 UI面板 | `明渠设计.py`, `styles.py`, `export_utils.py`, `formula_renderer.py`, `dxf_export.py`, `comparison.py`, `section_comparison.py`, `section_shapes.py`, `section_plotting.py` |
...
- `app_渠系计算前端/section_shapes.py` — 四类建筑物界面断面图共享几何适配层
- `app_渠系计算前端/section_plotting.py` — 四类建筑物界面断面图共享 matplotlib 绘图器
...
#### 5.4.1 梯形/矩形/复式梯形/U 形

- 单工况统一显示一张设计流量图；勾选且加大水深有效时，在同图叠加加大水位
- 多工况为每个成功工况一个小图，图内叠加设计水位和加大水位；水体填充仍表示设计水位
- 勾选加大流量且加大水深有效时，图内额外显示 `h加大=x.xxm` 竖向双向尺寸箭头，并自动放到既有尺寸标注外侧
- 轮廓线（黑色实线）
- 水面填充（浅蓝半透明）
- 底宽标注 B（灰色双箭头）
- 渠高标注 H（紫色双箭头）
...
- 单工况统一显示一张设计流量图；勾选且 `y_i` 有效时，在同图叠加加大水位
- 多工况为每个成功工况一个小图，勾选且 `y_i` 有效时叠加“加大水位”虚线和标注
- 多工况圆形断面同样显示 `h加大=x.xxm` 竖向双向尺寸箭头，水深来源为 `y_i`
- 轮廓圆（黑色实线）
- 水面弧面填充（浅蓝半透明）
- 直径标注 D（灰色双箭头）
- 水深标注 y（蓝色双箭头）
...
- U 形明渠弧底使用 AutoCAD 原生 `ARC`，不再使用多段直线折线近似。
...
| v1.6 | 2026-05-01 | 修复 U 形明渠 DXF 弧底：由多段直线拼接改为 AutoCAD 原生 `ARC`，便于 CAD 中按圆弧实体选择和编辑。 |
| v1.7 | 2026-05-01 | 工况对比水力表同步重点结果：圆形明渠补齐设计/加大水深、流速、流量、净空高度和净空比例，其他断面补充设计/加大渠道超高列。 |
| v1.8 | 2026-05-01 | 工况对比水力表删除“控制余量类型 / 控制余量”摘要列，界面、DXF 和 Word 统一以明确重点结果列展示安全余量。 |
| v1.9 | 2026-05-08 | 单工况界面断面图统一为单图叠加：设计水位保留水体填充和蓝色实线，加大水位在同图叠加虚线、文字和 `h加大` 标注；计算、DXF 和 Word 不变。 |
