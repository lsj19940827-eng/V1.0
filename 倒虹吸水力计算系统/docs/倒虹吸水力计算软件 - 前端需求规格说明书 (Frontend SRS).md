...
| 文件 | 职责 |
|------|------|
| `app_渠系计算前端/siphon/panel.py` | 主面板 `SiphonPanel`，含全部UI构建、参数联动、计算调用、导出、工况集成、示例数据管理，以及手工局部系数未采用时的非阻断提示；旧项目在唯一可恢复时会自动补回来源索引，无法唯一恢复时回退自动值；主界面上方 `ξ₁ / ξ₂` 只表示渐变段系数，结构段表中的进出水口系数只表示构件局部损失 |
...
示例身份只影响提示和持久化策略，不再覆盖真实几何显示；示例里的 ARC 节点与真实 DXF 导入数据一样，优先按圆弧真源采样绘制。
...
| 从 `from_dict()` 加载旧工况（仅有 `segments` / 旧 `plan_feature_points`） | 先自动迁移旧圆弧真源与纵断面节点，迁移失败才回退示例数据 |
| 工况切换时无纵断面数据 | 自动添加示例数据 |
| 用户编辑纵断面节点表 | 自动清除示例标志 |
| 用户编辑纵断面结构段 | 自动清除示例标志 |
| 保存工况（`to_dict()`） | 示例数据不保存（`_longitudinal_is_example=True` 时跳过节点） |
| 保存时标记 | `longitudinal_is_example: true` 标记到 JSON |
| 示例数据画布显示 | 只要节点里有真实圆弧元数据，就按圆弧绘制，不因示例身份退回折线 |
