...
4. **局部系数明细输出**：`verbose=True` 时必须逐项列出平面转弯、纵断面转弯和通用构件的最终采用 ξ；有公式或查表依据时同步输出过程，没有公式参数来源时明确标注“来自结构段表采用值”，最后展开 `Σξ_平面 / Σξ_纵断面 / Σξ_通用 / Σξ_local` 的加法式
5. **渐变段系数**：从 global_params 获取 `xi_inlet / xi_outlet`，仅用于 ΔZ₁ / ΔZ₃，不被结构段表中的进出水口系数替代
...
6. **详细计算过程输出**：当 `verbose=True` 时，在保留设计工况“步骤1~3”的基础上，追加独立的“步骤4：加大流量工况水头损失求解”块，完整输出 `Q加大 / v加大 / v₁加大 / v₂加大 / v₃加大 / ΔZ1加大 / hf加大 / hj加大 / 速度水头差加大 / ΔZ2加大 / ΔZ3加大 / ΔZ加大` 的采用值与公式代入过程；加大工况的 `hj加大` 沿用设计工况步骤2已展开的 `Σξ_local`，不重复输出同一组局部系数来源
...
| v3.7 | 2026-04-25 | **局部损失明细增强**：详细计算过程展开 `Σξ_local` 的平面转弯、纵断面转弯和通用构件来源，通用构件补充查表/公式或结构段表采用值说明；加大工况继续沿用同一合计值，避免重复堆叠来源说明。 |
