...
- `app_渠系计算前端/water_profile/water_profile_dialogs.py`：有压管道配置窗口，负责卡片、平面/纵断面数据、R/D 应用状态和窗口返回载荷。
...
- `推求水面线/utils/pressure_pipe_extractor.py`：保留命名组提取入口，同时提供窗口专用入口，把 xx管 匿名普通有压管道段也提出来。
