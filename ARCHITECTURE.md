...
- `app_渠系计算前端/water_profile/water_profile_dialogs.py`：有压管道配置窗口，负责 xx管 整线卡、连续承压链卡、分段卡、平面/纵断面数据、R/D 应用状态和窗口返回载荷。
...
- `water_profile_dialogs.py` 把整线纵断面按 `route_key` 收集后返回给 `panel.py`，并按连续承压链组织窗口展示。
