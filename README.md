...
应用内自动更新现在只对近版本提供补丁包；更老版本会直接下载全量包。补丁和全量安装都会在“校验安装环境”阶段继续显示细分状态，例如“正在统计安装目录大小（已扫描 N 个文件）”“正在解压完整安装包（x/y）”“正在校验补丁适用性（x/y）”，避免用户误以为卡死。
...
- 更新链路：`updater.py`、`update_helper.py`、`tools/build.py`、`tools/release.py` 共同负责版本检查、补丁/全量选择、独立安装窗口、`validate` 阶段细分进度、补丁兜底和正式发版。
...
- 全量更新安装窗口现在也会在“校验安装环境”阶段持续显示目录扫描和解压进度；补丁安装则继续显示补丁解压和适用性校验进度。
