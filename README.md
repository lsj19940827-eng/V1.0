...
- xx管 夹带隧洞导出现在按结构拆成上下两张表：上方普通渠道表继续跟随当前 `7/扩展项` 配置并承接隧洞，下方 `xx管` 表只保留 `有压管道 / 定向钻 / 顶管`。
- xx管 隧洞参数现在统一只认表1/Excel 当前值；弹窗只展示只读摘要和缺项提示，不再新增单独录入；圆拱直墙型只认 `B`，不再使用 `H`。
- xx管 下方专用表不再为隧洞单独展示底线或断面参数；涉及隧洞的复核和修改，一律回表1或 Excel 处理。
