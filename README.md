...
有压管道窗口现在重新按渠道级别收口：`xx管` 继续显示整线卡，`xx渠` 只显示当前有压管道分组，不再误把整条连续轴线带进弹窗。
...
- xx渠 的有压管道窗口已恢复为“只看当前分组”的口径，不再误显示整线卡；点击“开始计算”时，route 分段组装也不会再因为 identity helper 调用方式错误而报 `group` 参数缺失。
