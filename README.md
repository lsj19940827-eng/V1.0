...
应用内自动更新现在只对近版本提供补丁包；更老版本会直接下载全量包，补丁安装窗口也会显示“正在解压补丁包 / 正在校验补丁适用性（x/y）”，避免用户误以为卡死。
...
- 更新链路：`updater.py`、`update_helper.py`、`tools/build.py`、`tools/release.py` 共同负责版本检查、补丁/全量选择、独立安装窗口、补丁兜底和正式发版。
...
- 运行本次复式梯形回归：`$env:PYTHONPATH='D:\V1.0;D:\V1.0\calc_渠系计算算法内核'; D:\V1.0\.venv\Scripts\python.exe -m pytest tests/test_open_channel_appendix_e_table_unit.py tests/test_open_channel_multi_case_refresh_unit.py tests/test_batch_import_dimension_preserve_unit.py tests/test_shared_data_manager_vmax_alias_unit.py tests/test_open_channel_compound_trapezoid_kernel_unit.py tests/test_open_channel_compound_trapezoid_panel_unit.py tests/test_open_channel_compound_trapezoid_dxf_unit.py tests/test_batch_compound_trapezoid_unit.py tests/test_compound_trapezoid_type_support_unit.py tests/test_compound_trapezoid_shared_hydraulic_unit.py tests/test_water_profile_coord_precision_unit.py -q`
...
- 2026-04-09：本次为现有明渠能力扩展，README 已存在搜索记录，按仓库约定跳过 skills.sh 和 GitHub 额外搜索，直接基于现有梯形链路扩展 `复式梯形`。
...
- 自动更新补丁策略已收紧到近版本链路：当前只给 `1.1.9+` 提供补丁；如果补丁删除文件过多或总覆盖量过大，构建阶段会直接跳过补丁，只发布全量包。
