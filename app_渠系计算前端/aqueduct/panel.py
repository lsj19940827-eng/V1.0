...
from app_渠系计算前端.aqueduct.dxf_export import (
    export_aqueduct_dxf,
    draw_aqueduct_dxf_on_msp,
    draw_aqueduct_comparison_table,
)
from app_渠系计算前端.aqueduct.comparison import (
    AQUEDUCT_COMPARISON_SPEC,
    build_aqueduct_comparison_tables,
)
...
class AqueductPanel(QWidget):
    ...
    def _refresh_comparison_tables(self):
        ...
        if not hasattr(self, "comparison_hydraulic_table"):
            return
        tables = build_aqueduct_comparison_tables(getattr(self, "_all_results", []))
        fill_comparison_table(
            self.comparison_hydraulic_table,
            AQUEDUCT_COMPARISON_SPEC.hydraulic_columns,
        ...
        if tables.hydraulic_rows:
            self._set_comparison_hint("已汇总成功计算的工况；槽身总高按含拉杆总高口径显示。")
        else:
    ...
    def _export_single_dxf_entry(self, entry, scale_denom=None):
        ...
        export_single_case_dxf(
            ...
            scale,
            draw_aqueduct_dxf_on_msp,
            draw_summary_table=draw_aqueduct_comparison_table,
        )
        return filepath
