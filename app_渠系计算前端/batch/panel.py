...
class BatchPanel(QWidget):
    ...
    def _build_input_area(self, parent):
        ...
        ig.addWidget(QLabel("渠道类型:")); self.channel_level_combo = ComboBox(); self.channel_level_combo.addItems(CHANNEL_LEVEL_OPTIONS); self.channel_level_combo.setCurrentText("支渠"); self.channel_level_combo.setFixedWidth(80); ig.addWidget(self.channel_level_combo)
    ...
    def _get_template_path(self, template_key: str = "blank"):
        ...
        if getattr(sys, 'frozen', False):
            ...
            # __file__ = app_渠系计算前端/batch/panel.py → 需要向上3层到V1.0/
    ...
    def _add_sample_data_6(self):
        """加载示例六：合作干渠数据（从data目录读取xlsx文件）"""
        path = self._get_sample6_path()
        if not os.path.exists(path):
            fluent_info(self, "错误", f"未找到示例六文件：\n{path}")
            return
        self._do_load_from_filepath(path, is_sample=True,
                                    sample_title="示例六",
                                    sample_desc="合作干渠示例数据")

    def _get_sample6_path(self):
        """获取示例六xlsx文件路径（兼容开发环境和打包环境）"""
        if getattr(sys, 'frozen', False):
            base = sys._MEIPASS
        else:
            base = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
