...
def _temp_test_dir(prefix: str) -> str:
    base = Path(_pkg_root) / ".pytest_tmp"
    base.mkdir(parents=True, exist_ok=True)
    return tempfile.mkdtemp(prefix=prefix, dir=str(base))

def _project_path(temp_dir):
    return str(Path(temp_dir) / f"test_project_{uuid4().hex}.ppipe")
...
def test_single_siphon_panel_auto_confirm():
    ...
    temp_dir = _temp_test_dir("siphon-unified-confirm-")
    try:
        project_path = _project_path(temp_dir)
...
def test_autosave_migration(monkeypatch):
    ...
    try:
        ...
        autosave_path = Path(temp_dir) / "siphon_autosave.json"
        ...
        monkeypatch.setattr(panel, "_get_autosave_path", lambda: str(autosave_path))
