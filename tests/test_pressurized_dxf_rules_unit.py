...
def _make_pressure_pipe_summary_node(
    flow_section,
    station_mc,
    water_level,
    structure_text,
    *,
    ...
    if is_pressure_pipe is None:
        is_pressure_pipe = structure_text == "有压管道"
    return SimpleNamespace(
        flow_section=flow_section,
        station_MC=station_mc,
        water_level=water_level,
        name=name,
        in_out=SimpleNamespace(value=in_out_text),
        structure_type=SimpleNamespace(value=structure_text),
        is_transition=False,
...
def _bind_pressure_pipe_summary_methods(dummy_panel):
    """给测试桩对象绑定压力管道摘要所需方法。"""
    for name in (
        "_collect_pressure_pipe_export_targets",
        "_build_pressure_pipe_characteristic_boundary_summary_from_targets",
    ...
    dummy_panel._resolve_pressure_pipe_target_station_range = (
        lambda target: panel_mod.WaterProfilePanel._resolve_pressure_pipe_target_station_range(target)
    )
    ...
    dummy_panel._get_pressure_pipe_summary_in_out_text = (
        panel_mod.WaterProfilePanel._get_pressure_pipe_summary_in_out_text
    )
    dummy_panel._make_pressure_pipe_export_target = (
        panel_mod.WaterProfilePanel._make_pressure_pipe_export_target
    )
    dummy_panel._coerce_pressure_pipe_row_index = (
        panel_mod.WaterProfilePanel._coerce_pressure_pipe_row_index
    )
    dummy_panel._coerce_pressure_pipe_finite_float = (
        panel_mod.WaterProfilePanel._coerce_pressure_pipe_finite_float
    )
...
def test_panel_pressure_pipe_characteristic_summary_merges_tunnel_subtypes_and_accumulates_lengths():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("3", 0.0, 512.3, "有压管道"),
            _make_pressure_pipe_summary_node("3", 100.0, 511.9, "隧洞-圆形", name="一号隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("3", 400.0, 511.3, "隧洞-马蹄形Ⅰ型", name="一号隧洞", in_out_text="出"),
            _make_pressure_pipe_summary_node("3", 650.0, 511.0, "定向钻", name="一号定向钻", in_out_text="进"),
            _make_pressure_pipe_summary_node("3", 850.0, 510.2, "定向钻", name="一号定向钻", in_out_text="出"),
            _make_pressure_pipe_summary_node("3", 900.0, 510.0, "顶管", name="一号顶管", in_out_text="进"),
            _make_pressure_pipe_summary_node("3", 1050.0, 509.7, "顶管", name="一号顶管", in_out_text="出"),
        ],
...
def test_panel_pressure_pipe_characteristic_summary_falls_back_to_flow_scan_without_boundary_metadata():
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "三清庙", "flow_section": 1}],
    )

    assert summary["1"]["total_length"] == 600.0
    assert summary["1"]["start_water_level"] == 407.318
    assert summary["1"]["end_water_level"] == 374.517


@pytest.mark.parametrize("channel_level", ["支管", "干管"])
def test_panel_pressure_pipe_characteristic_summary_single_flow_xxpipe_without_prefix_keeps_pressure_boundary_water_levels(channel_level):
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("1", 200.0, 377.659, "有压管道", name="三清庙", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 600.0, 374.517, "有压管道", name="三清庙", in_out_text="出"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level=channel_level),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
...
def test_panel_pressure_pipe_characteristic_summary_includes_plain_pressure_pipe_segments_in_total_length():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("1", 0.0, 397.16, "有压管道"),
            _make_pressure_pipe_summary_node("1", 200.0, 394.8, "定向钻", name="穿路段一", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 500.0, 391.8, "定向钻", name="穿路段一", in_out_text="出"),
            _make_pressure_pipe_summary_node("1", 700.0, 389.6, "有压管道"),
            _make_pressure_pipe_summary_node("1", 800.0, 388.9, "顶管", name="穿路段二", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 1000.0, 386.5, "顶管", name="穿路段二", in_out_text="出"),
            _make_pressure_pipe_summary_node("1", 1200.0, 384.203, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    assert summary["1"]["total_length"] == 1200.0
...
def test_panel_pressure_pipe_characteristic_summary_uses_boundary_metadata_for_noncontinuous_cross_flow_sections():
    ...
    assert summary["2"]["total_length"] == 200.0
    assert summary["2"]["start_water_level"] == 404.8
    assert summary["2"]["end_water_level"] == 403.6
...
def test_panel_pressure_pipe_characteristic_summary_keeps_separate_boundary_levels_when_not_continuous():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("1", 0.0, 410.0, "有压管道"),
            _make_pressure_pipe_summary_node("1", 120.0, 408.6, "定向钻", name="一号定向钻", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 200.0, 407.9, "定向钻", name="一号定向钻", in_out_text="出"),
            _make_pressure_pipe_summary_node("1", 260.0, 407.4, "有压管道"),
            _make_pressure_pipe_summary_node("", 300.0, 406.1, "明渠"),
            _make_pressure_pipe_summary_node("2", 320.0, 405.2, "有压管道"),
            _make_pressure_pipe_summary_node("2", 420.0, 404.8, "顶管", name="二号顶管", in_out_text="进"),
            _make_pressure_pipe_summary_node("2", 500.0, 404.1, "顶管", name="二号顶管", in_out_text="出"),
            _make_pressure_pipe_summary_node("2", 640.0, 403.6, "有压管道"),
...
def test_panel_pressure_pipe_characteristic_summary_uses_next_flow_start_as_previous_flow_end():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("1", 0.0, 410.0, "有压管道"),
            _make_pressure_pipe_summary_node("1", 7816.815, 401.2, "有压管道"),
            _make_pressure_pipe_summary_node("2", 7938.468, 400.7, "有压管道"),
            _make_pressure_pipe_summary_node("2", 10800.0, 395.4, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    assert summary["1"]["total_length"] == 7938.468
    assert summary["2"]["total_length"] == 2861.532


def test_panel_pressure_pipe_characteristic_summary_skips_prepressure_tunnel_for_xxqu():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("5", 100.0, 410.3, "隧洞-圆形", name="前置隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("5", 400.0, 409.2, "隧洞-圆形", name="前置隧洞", in_out_text="出"),
            _make_pressure_pipe_summary_node("5", 500.0, 408.9, "有压管道"),
            _make_pressure_pipe_summary_node("5", 800.0, 406.8, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支渠"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "第五流量段", "flow_section": 5}],
    )

    assert summary["5"]["tunnel_count"] == 0
    assert summary["5"]["tunnel_length"] == 0.0
    assert summary["5"]["total_length"] == 300.0


def test_panel_pressure_pipe_characteristic_summary_counts_postpressure_tunnel_for_xxqu():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("6", 0.0, 412.0, "有压管道"),
            _make_pressure_pipe_summary_node("6", 100.0, 411.1, "隧洞-圆形", name="后置隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("6", 380.0, 409.8, "隧洞-圆形", name="后置隧洞", in_out_text="出"),
            _make_pressure_pipe_summary_node("6", 520.0, 409.1, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支渠"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "第六流量段", "flow_section": 6}],
    )

    assert summary["6"]["tunnel_count"] == 1
    assert summary["6"]["tunnel_length"] == 280.0
    assert summary["6"]["total_length"] == 520.0


def test_panel_pressure_pipe_characteristic_summary_skips_terminal_tunnel_for_xxqu():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("6", 0.0, 412.0, "有压管道"),
            _make_pressure_pipe_summary_node("6", 200.0, 410.8, "有压管道"),
            _make_pressure_pipe_summary_node("6", 300.0, 410.1, "隧洞-圆形", name="末尾隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("6", 500.0, 409.0, "隧洞-圆形", name="末尾隧洞", in_out_text="出"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支渠"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    assert summary["6"]["tunnel_count"] == 0
    assert summary["6"]["tunnel_length"] == 0.0
    assert summary["6"]["total_length"] == 300.0


def test_panel_pressure_pipe_characteristic_summary_skips_terminal_tunnel_after_directional_drill_for_xxqu():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("7", 0.0, 415.0, "定向钻", name="先导钻", in_out_text="进"),
            _make_pressure_pipe_summary_node("7", 80.0, 414.4, "定向钻", name="先导钻", in_out_text="出"),
            _make_pressure_pipe_summary_node("7", 120.0, 414.0, "隧洞-圆形", name="后置隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("7", 360.0, 412.8, "隧洞-圆形", name="后置隧洞", in_out_text="出"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支渠"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "第七流量段", "flow_section": 7}],
    )

    assert summary["7"]["directional_drill_count"] == 1
...
def test_panel_pressure_pipe_characteristic_summary_uses_directional_drill_and_jacking_as_xxqu_tunnel_gates():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("7", 0.0, 415.0, "定向钻", name="先导钻", in_out_text="进"),
            _make_pressure_pipe_summary_node("7", 80.0, 414.4, "定向钻", name="先导钻", in_out_text="出"),
            _make_pressure_pipe_summary_node("7", 120.0, 414.0, "隧洞-圆形", name="中间隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("7", 360.0, 412.8, "隧洞-圆形", name="中间隧洞", in_out_text="出"),
            _make_pressure_pipe_summary_node("7", 420.0, 412.2, "顶管", name="后续顶管", in_out_text="进"),
            _make_pressure_pipe_summary_node("7", 520.0, 411.7, "顶管", name="后续顶管", in_out_text="出"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支渠"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    assert summary["7"]["directional_drill_count"] == 1
    assert summary["7"]["directional_drill_length"] == 80.0
    assert summary["7"]["tunnel_count"] == 1
    assert summary["7"]["tunnel_length"] == 240.0
...
def test_panel_pressure_pipe_characteristic_summary_keeps_xxpipe_tunnel_counting_rule():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("8", 60.0, 420.0, "隧洞-圆形", name="整线前置隧洞", in_out_text="进"),
            _make_pressure_pipe_summary_node("8", 260.0, 419.2, "隧洞-圆形", name="整线前置隧洞", in_out_text="出"),
            _make_pressure_pipe_summary_node("8", 320.0, 418.9, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(channel_level="支管"),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "第八流量段", "flow_section": 8}],
    )

    assert summary["8"]["tunnel_count"] == 1
...
@pytest.mark.parametrize(
    ...
    [
        ("隧洞-圆形", "tunnel_count", "tunnel_length"),
        ("定向钻", "directional_drill_count", "directional_drill_length"),
        ("顶管", "jacking_count", "jacking_length"),
    ],
)
def test_panel_pressure_pipe_characteristic_summary_does_not_extend_building_length_past_outlet(
    structure_text,
    count_key,
    length_key,
):
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("2", 0.0, 402.1, "有压管道"),
            _make_pressure_pipe_summary_node("2", 100.0, 401.0, structure_text, name="测试建筑物", in_out_text="进"),
            _make_pressure_pipe_summary_node("2", 200.0, 400.0, structure_text, name="测试建筑物", in_out_text="出"),
            _make_pressure_pipe_summary_node("2", 260.0, 399.2, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    summary = panel_mod.WaterProfilePanel.get_pressure_pipe_characteristic_export_summary(
        dummy_panel,
        rows=[{"name": "第二流量段", "flow_section": 2}],
    )

    assert summary["2"]["total_length"] == 260.0
    assert summary["2"][count_key] == 1
    assert summary["2"][length_key] == 100.0


def test_panel_pressure_pipe_characteristic_summary_formats_two_directional_drills_by_inlet_outlet_mc():
    dummy_panel = SimpleNamespace(
        calculated_nodes=[
            _make_pressure_pipe_summary_node("1", 957.439, 410.2, "有压管道"),
            _make_pressure_pipe_summary_node("1", 1007.440, 409.8, "定向钻", name="磨盘寨", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 1901.261, 406.7, "定向钻", name="磨盘寨"),
            _make_pressure_pipe_summary_node("1", 1916.260, 406.5, "定向钻", name="磨盘寨", in_out_text="出"),
            _make_pressure_pipe_summary_node("1", 1949.129, 406.3, "有压管道"),
            _make_pressure_pipe_summary_node("1", 2236.090, 404.6, "定向钻", name="观音岩", in_out_text="进"),
            _make_pressure_pipe_summary_node("1", 2331.140, 404.1, "定向钻", name="观音岩"),
            _make_pressure_pipe_summary_node("1", 2879.430, 401.0, "定向钻", name="观音岩"),
            _make_pressure_pipe_summary_node("1", 2898.430, 400.8, "定向钻", name="观音岩", in_out_text="出"),
            _make_pressure_pipe_summary_node("1", 2993.004, 400.2, "有压管道"),
        ],
        _build_settings=lambda: SimpleNamespace(),
    )
    _bind_pressure_pipe_summary_methods(dummy_panel)
    ...
    assert summary["1"]["directional_drill_count"] == 2
    assert summary["1"]["directional_drill_length"] == pytest.approx(1571.16, abs=1e-9)
    assert summary_mod._pressure_pipe_building_length_km(summary["1"]["directional_drill_length"]) == 1.571


def test_merge_pressure_pipe_export_rows_by_flow_section_hides_building_characteristics_when_xxqu_tunnel_is_filtered():
    rows = [
        {
            "name": "第五段管道",
            "flow_section": 5,
            "display_name": "第五段管道-第五流量段",
            "pipe_material": "球墨铸铁管",
            "DN_mm": 1200,
            "structure_kind": "pressure_pipe",
            "Q": 1.2,
    ...
    panel = SimpleNamespace(
        get_pressure_pipe_characteristic_export_summary=lambda export_rows=None: {
            "5": {
                "total_length": 500.0,
                "start_water_level": 410.3,
                "end_water_level": 408.9,
                "tunnel_count": 0,
                "tunnel_length": 0.0,
                "directional_drill_count": 0,
                "directional_drill_length": 0.0,
...
def test_pressure_pipe_summary_table_uses_shared_boundary_water_levels_after_merge():
    rows = [
        {
            "name": "第一段管道",
            "flow_section": 1,
    ...
    assert table_rows[0][8] == 405.2
    assert table_rows[1][0] == "第二流量段"
    assert table_rows[1][7] == 405.2
    assert table_rows[1][8] == 403.6


def test_merge_pressure_pipe_export_rows_by_flow_section_prefers_pressure_pipe_boundary_summary():
    rows = [
        {
            "name": "三清庙",
            "flow_section": 1,
    ...
    assert len(merged) == 1
    assert merged[0]["total_length"] == 400.0
    assert merged[0]["plan_total_length"] == 400.0
    assert merged[0]["start_water_level"] == 377.659
    assert merged[0]["end_water_level"] == 374.517
    assert table_rows[0][3] == pytest.approx(0.4)
