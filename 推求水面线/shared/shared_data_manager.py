...
用于在多流量段批量计算结果和推求水面线系统之间传递数据。
当用户在批量计算标签页完成计算后，计算结果会被注册到这个管理器中。
然后在推求水面线系统中，用户可以直接从已有的批量计算结果中导入断面参数。
"""
...
class SectionResult:
    """断面计算结果"""
    source: str              # 来源标识（如"批量计算-1"、"批量计算-2"等）
...
class SharedDataManager:
    ...
    def _extract_section_result(self, source: str, result: Dict) -> Optional[SectionResult]:
        ...
        try:
            ...
            # 闸·分水类特殊处理：不参与断面计算，只记录位置和名称信息
            is_diversion_gate = result.get('is_diversion_gate', False) or "闸" in section_type or "分水" in section_type
            ...
            section_result.X = result.get('X_design', result.get('wetted_perimeter',
                              result.get('P_design', result.get('P_d', 0))))
            section_result.R_hydraulic = result.get('R_hyd_design', result.get('hydraulic_radius',
                                        result.get('R_design', result.get('R_d', 0))))
            
            # 提取加大流量工况（兼容圆形/隧洞的键名 Q_inc, y_i, V_i）
            section_result.Q_max = result.get('Q_increased', result.get('Q_max', result.get('Q_inc', 0)))
            section_result.h_max = result.get('h_increased', result.get('h_max', result.get('y_i', 0)))
            section_result.V_max = result.get('V_increased', result.get('V_max', result.get('V_i', 0)))
            ...
            # 提取结构高度（用于计算渠顶高程 = 渠底高程 + 结构高度）
            # 不同结构类型的结构高度取值不同：
            #   明渠-梯形/矩形: 渠道高度 h_prime
            #   明渠-圆形/隧洞-圆形: 设计直径 D
            #   渡槽-U形/矩形: 槽身总高 H_total
            #   隧洞-圆拱直墙型/矩形暗涵: 总高 H_total
            #   隧洞-马蹄形: 2R
    ...
    def get_summary(self) -> str:
        ...
        if self._batch_results:
            return f"【批量计算结果】共 {len(self._batch_results)} 条"
        return "暂无计算结果"
